...
private:
	...
	int modType = 0;
	enum class modTypeEnum { Flanger,Vibrato,Chorus };	// to compare: if(compareEnumToInt(modTypeEnum::Flanger, modType)) etc... 

	int enableMod = 0;
	enum class enableModEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(enableModEnum::SWITCH_OFF, enableMod)) etc... 
