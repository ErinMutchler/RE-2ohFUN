...
bool Rafx2Plugin::processRafx2Message(const Rafx2MessageInfo& messageInfo)
{
	...
	switch (messageInfo.message)
	{
		...
		case PLUGINGUI_USER_CUSTOMOPEN:
		{
			...
			// --- create GUI
			UTF8StringPtr _xmlFile = "PluginGUI.uidesc";
			pluginGUI = new PluginGUI(_xmlFile);
			if (!pluginGUI) return false;

			bool opened = ((PluginGUI*)pluginGUI)->open("Editor", guiInfo->window, PluginParameterPtr, kHWND, guiPluginConnector, nullptr);
			...
			if (opened)
			{
				...
				((PluginGUI*)pluginGUI)->getSize(width, height);
				guiInfo->width = width;
				guiInfo->height = height;

				((PluginGUI*)pluginGUI)->setGUIWindowFrame(guiInfo->guiWindowFrame);
			}
			return true;
		}
		...
		case PLUGINGUI_USER_CUSTOMCLOSE:
		{
			if (pluginGUI)
			{
				...
				// --- close it
				((PluginGUI*)pluginGUI)->close();

				// --- self-destruct
				PluginGUI* oldGUI = ((PluginGUI*)pluginGUI);
				pluginGUI = 0;
				oldGUI->forget();
			}
			return true;
...
{
	/*if (!pluginGUIConnector) return;
	if (!pluginCore) return;

	PluginParameter* piParam = pluginCore->getPluginParameterByControlID(controlID);
	if (piParam)
	{
		pluginGUIConnector->syncGUIControl(piParam->getControlID());
	}*/
}
